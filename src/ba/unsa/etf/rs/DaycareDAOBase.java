...
public class DaycareDAOBase implements DaycareDAO {
    ...
    private PreparedStatement updateChildGradeNullStatement;
    ...
    private PreparedStatement getChildrenWithoutGradeStatement;
    ...
    private DaycareDAOBase() {
        try {
            ...
            updateChildGradeNullStatement = connection.prepareStatement("UPDATE children SET grade_id=null WHERE person_id=?");
            ...
            getChildrenWithoutGradeStatement = connection.prepareStatement("SELECT persons.id, first_name, last_name, address, jmbg, place_of_birth, date_of_birth, gender, first_parent_id, second_parent_id FROM persons,children WHERE children.person_id = persons.id AND children.grade_id IS NULL");
    ...
    public void addChild(Child child) {
        try {
            ...
            addChildStatement.setInt(1, personID);
    ...
    public void updateChild(Child child) {
        try {
            updatePerson(child);
            updateChildStatement.setInt(1, child.getFirstParent().getId());
            updateChildStatement.setInt(2, child.getSecondParent().getId());
            updateChildStatement.setInt(3, child.getId());
    ...
    public void addTeacher(Teacher teacher) {
        try {
            ...
            addTeacherStatement.setInt(1, personID);
    ...
    public void addParent(Parent parent) {
        try {
            ...
            addParentStatement.setInt(1, personID);
    ...
    @Override
    public ArrayList<Child> getChildrenWithoutGrade() {
        ArrayList<Child> children = new ArrayList<>();
        try {
            ResultSet rs = getChildrenWithoutGradeStatement.executeQuery();
            while (rs.next()) {
                Parent firstParent = getParent(rs.getInt(9));
                Parent secondParent = getParent(rs.getInt(10));
                Child child = getChildFromResultSet(rs, firstParent, secondParent, null); // TODO: Child Activity
    ...
    @Override
    public void updateGradeChildren(Grade grade, ObservableList<Child> children) {
        for (Child child : children) {
            try {
                updateChildGradeStatement.setInt(1, grade.getId());
                updateChildGradeStatement.setInt(2, child.getId());
                updateChildGradeStatement.executeUpdate();
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
        }
    ...
    @Override
    public void removeGradeChildren(ObservableList<Child> children) {
        for (Child child : children) {
            try {
                updateChildGradeNullStatement.setInt(1, child.getId());
                updateChildGradeNullStatement.executeUpdate();
            } catch (SQLException throwables) {
                throwables.printStackTrace();
            }
        }
